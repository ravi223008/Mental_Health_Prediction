# Data Pre.py — cleaning + privacy-safe outputs + stratified 70-15-15 split + train-only 5-fold CV
# -*- coding: utf-8 -*-
...
# Runtime knobs
# =========================
STRICT   = False   # True = fail fast on unmapped values; False = warn and set to NA
K_MIN    = 5       # k-anonymity threshold for public views
SEED     = 760      # RNG seed for RR3 & splits
LOW_FREQ = 20      # Merge public categories with count < LOW_FREQ into "Other"

# =========================
# Helpers (normalization, mapping, privacy)
# =========================
def read_csv_robust(path: Path):
    """Try several encodings; print which one worked."""
...
def map_with_assert(s: pd.Series, mapping: dict, name=None) -> pd.Series:
    """Map tokens to integers per vocabulary; optionally fail fast on unknowns."""
    x = norm_token(s)
    # lightweight synonyms to improve coverage
    mapping_ext = {
        # smoking
...
# =========================
# Splits & 5-fold CV (pure NumPy; no sklearn)
# =========================
def _target_from_full(clean_full: pd.DataFrame, label_maps_path: Path|None=None) -> np.ndarray:
    """
    Return y in {0,1} preferring textual Mental_Health_Condition (Yes/No).
    Falls back to *_lbl + label_mappings.json if needed.
    """
    tcol = col(clean_full, "Mental_Health_Condition","MHC","Condition")
    if tcol:
        return (clean_full[tcol].astype("string").str.strip().str.lower() == "yes").astype(int).to_numpy()
    lbl = col(clean_full, "Mental_Health_Condition_lbl")
    if lbl and label_maps_path and label_maps_path.exists():
        try:
            maps = json.loads(label_maps_path.read_text(encoding="utf-8"))
            inv = {int(v): str(k).lower() for k,v in maps.get("Mental_Health_Condition", {}).items()}
            y = clean_full[lbl].to_numpy()
            return np.array([1 if inv.get(int(v), "")=="yes" else 0 for v in y], dtype=int)
        except Exception as e:
            print("[WARN] Failed to derive target from label_mappings:", e)
    raise RuntimeError("Cannot locate target 'Mental_Health_Condition' (text or *_lbl).")

def _stratified_3way(y: np.ndarray, p_train=0.70, p_val=0.15, p_test=0.15, seed=SEED):
    """
    Stratified split into train/val/test by class proportions of y in {0,1}.
    ...
    for cls in [0,1]:
        ...
        rng.shuffle(idx)
        n_c = len(idx)
        n_train = int(np.floor(p_train * n_c))
        n_val   = int(np.floor(p_val   * n_c))
        n_test  = n_c - n_train - n_val
        train[idx[:n_train]] = True
        val[idx[n_train:n_train+n_val]] = True
        test[idx[n_train+n_val:]] = True
    rest = (~train & ~val & ~test)
    if rest.any():  # just in case due to rounding
        test[rest] = True
    return train, val, test

def _stratified_kfold_on_train(y: np.ndarray, train_mask: np.ndarray, k=5, seed=SEED):
    """
    Stratified k-fold assignment for training subset only.
    Returns cv_fold array length n with {0..k-1} on train, -1 otherwise.
    """
    rng = np.random.default_rng(seed)
    n = len(y)
    cv = np.full(n, -1, dtype=int)
    for cls in [0,1]:
        idx_train_cls = np.where((y==cls) & train_mask)[0]
        rng.shuffle(idx_train_cls)
        folds = np.array_split(idx_train_cls, k)
        for fid, arr in enumerate(folds):
            cv[arr] = fid
    return cv

def make_splits_artifact(processed_dir: Path, seed=SEED, k=5,
                         p_train=0.70, p_val=0.15, p_test=0.15):
    """
    Produce splits_70_15_15_k{k}.csv with:
      row_id, split {train/val/test}, cv_fold {0..k-1 on train, -1 else}
    """
    cf = pd.read_csv(processed_dir / "clean_full.csv")
    maps_path = processed_dir / "label_mappings.json"
    y = _target_from_full(cf, maps_path)

    train_mask, val_mask, test_mask = _stratified_3way(y, p_train, p_val, p_test, seed)
    cv_fold = _stratified_kfold_on_train(y, train_mask, k, seed)

    split = np.full(len(cf), "unassigned", dtype=object)
    split[train_mask] = "train"
    split[val_mask]   = "val"
    split[test_mask]  = "test"
    assert (split != "unassigned").all()
    assert (cv_fold[split!="train"] == -1).all()

    out = pd.DataFrame({"row_id": np.arange(len(cf), dtype=int),
                        "split": split, "cv_fold": cv_fold})
    out_path = processed_dir / f"splits_70_15_15_k{k}.csv"
    out.to_csv(out_path, index=False)

    counts = pd.Series(split).value_counts().to_dict()
    fold_counts = pd.Series(cv_fold[cv_fold>=0]).value_counts().sort_index().to_dict()
    print(f"[INFO] Split counts: {counts}")
    print(f"[INFO] Train fold sizes: {fold_counts}")
    return out_path, counts, fold_counts

# =========================
# Cleaning + privacy-safe outputs
# =========================
def run_cleaning(args):
    ...
    # ---- Range clipping (exclude Stress_Level)
    bounds = {}
    ...
    # ---- clean_full (analysis-friendly)
    clean_full = df.copy()

    # ---- Label encode & store mappings
    label_maps = {}
    ...
    # Simple imputation (numeric view only)
    ...
    # Example RR3 table (Gender × Mental_Health_Condition)
    if {"Gender_norm","Mental_Health_Condition"}.issubset(cf_pub.columns):
        ...
        tab_supp = tab.mask(tab < 3, 0)  # small cell suppression
    ...
    # ===== (Optional) make splits =====
    split_info = ""
    if not args.no_splits:
        split_path, counts, fold_counts = make_splits_artifact(out_dir, seed=SEED, k=args.kfolds,
                                                               p_train=args.p_train, p_val=args.p_val, p_test=args.p_test)
        split_info = f"Splits written: {split_path.name} | counts={counts} | train folds={fold_counts}"
    else:
    ...
    if qcols: lines.append((viol_txt or "") + f" | LOW_FREQ merge threshold={LOW_FREQ}")
    lines.append(split_info)
    ...
    print(" -", summ_path)
    if not args.no_splits:
        print(" -", out_dir / f"splits_70_15_15_k{args.kfolds}.csv")
...
# =========================
def main():
    parser = argparse.ArgumentParser(description="Team-style data cleaning + privacy-safe outputs + stratified splits")
    ...
    # split controls
    parser.add_argument("--no_splits", action="store_true", help="skip generating 70-15-15 + 5-fold labels")
    parser.add_argument("--kfolds", type=int, default=5, help="number of CV folds on training set")
    parser.add_argument("--p_train", type=float, default=0.70, help="train proportion (stratified)")
    parser.add_argument("--p_val",   type=float, default=0.15, help="validation proportion (stratified)")
    parser.add_argument("--p_test",  type=float, default=0.15, help="test proportion (stratified)")
    args = parser.parse_args()
    # sanity on proportions
    tot = round(args.p_train + args.p_val + args.p_test, 6)
    if abs(tot - 1.0) > 1e-6:
        raise ValueError(f"Split proportions must sum to 1.0 (got {tot})")
    run_cleaning(args)
